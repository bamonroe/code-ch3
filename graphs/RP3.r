...
domain <- data.frame(r=seq(from=xbot, to=xtop, by=.001))
...
area <- sapply(rvals, function(x){
			uniroot.all(solve,c(-10,1),F1=UBuy,F2=UA,r2=x)
		})

# Set this up in a reasonable dataframe
area0 <- data.frame(
				x=matrix(c(area[1,],area[1,],area[2,],area[2,])),
				y=c(rep(ybot,ncol(area)), rep(ytop,ncol(area)) , rep(ytop,ncol(area)) , rep(ybot,ncol(area))),
				id=1:ncol(area), 
				layer=rvals
				)
...
area0$layer <- factor(area0$layer)

# Order based on ID just to make it look nice
area0 <- area0[order(area0$id),]



# Make the first element of rvals the r value which indicates indifference
#x0 <- c(x<-uniroot.all(solve,c(-10,1),F1=UBuy,F2=UA))
x0 <- c(.55)
x0 <- rvals
...
hl$layer=factor(hl$x[which(hl$x < median(hl$x))])

hl$x <- sapply(hl$x , function(x){
	r <- uniroot.all(solve,c(-10,1),F1=UBuy,F2=UA,r2=x)
	ifelse(abs(r[1]-x) < abs(r[2]-x), r[1], r[2])
 })
...
# Add the polygons
p <- p + geom_polygon(data=area0, aes(x=x, y=y,group=id, fill=layer ),alpha=1)

#p <- p + geom_ribbon(aes(ymax=UA,ymin=ybot))
...
p <- p +
    ...
    scale_x_continuous(
    				   limits = c(xbot, xtop),
    				   breaks=c(seq(from=xbot, to=xtop ,by=.15))
    				   ) +
    scale_y_continuous(
    				   limits = c(ybot, ytop),
