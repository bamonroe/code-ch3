...
while(TRUE){
	...
	if(dnum %% 2 == 1){
		...
		tm <- runif(1,min=tm.min,max=tm.max)
		ts.max <- ifelse(tm>.5,.75*(1-tm),.75*tm)
		ts <- runif(1,min=ts.min,max=ts.max)
	}
	else{
...
#			rm <- runif(1,min=rm.min,max=rm.max)
	...
	# Tremble will use a beta distribution, so need to back out shape1 and
	# shape2 parameters
	a <- ((1-tm)/(ts^2) - 1/tm) * tm^2
	b <- a * (1/tm - 1)

	sim <- matrix(  c(rnorm(snum,mean = rm, sd = rs), rgamma(snum,shape = k, scale = t) , rbeta(snum,shape1 = a, shape2 = b) ) , nrow=snum  )

	# Given the very wide bounds we've made for ourselves, occasionally we get 
	...
	save(D ,file=paste("Dat",dnum,"-SU-TR.Rda",sep=""))
    rm(D)
