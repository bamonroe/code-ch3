...
getSample <- function (rm,rs,um,us){
...
getAgg <- function(theta){
	...
	D <- getSample(rm=theta[1],rs=theta[2],um=theta[3],us=theta[4])
	...
	c(WW,rm=theta[1],rs=theta[2],um=theta[3],us=theta[4])
...
Theta <- data.frame(matrix(c(
	...
	rs = halton(start,end,7)*rsdiff + rs.min,	# for mu
	um = halton(start,end,11)*umdiff + um.min,	# for r
...
NN <- mclapply( Theta, getAgg,mc.cores=cores)
NN <- do.call(rbind,NN)
NN <- data.frame(NN)
...
#save(MM,file=paste("Sam",S,"-sim",snum,".Rda",sep=""))
