...
# Grab the top 10 most like choice patterns
top.ten <- E %>%
...
write.csv(top.ten, file = fname)

# Grab all of the Consistent choice patterns
cons <- E %>%
...
# Grab the top 10 Most likely LightMSB Choice patterns
lmsb <- E %>%
	filter(Type=="Light MSB") %>%
	select(Pattern,PC,EE,WP,WC,E.0,E.1) %>%
	arrange(desc(PC)) %>%
	top_n(10,wt=PC)
