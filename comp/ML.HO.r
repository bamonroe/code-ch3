...
ML.EUT <- function(par){
	...
	UA <- dd$pA0 * rr(dd$A0,r) + dd$pA1 * rr(dd$A1,r) + dd$pA2 * rr(dd$A2,r)
	UB <- dd$pB0 * rr(dd$B0,r) + dd$pB1 * rr(dd$B1,r) + dd$pB2 * rr(dd$B2,r)
...
ML.RDU <- function(par){
	...
	wA2 <- pw(dd$pA2,a,b)
	wA1 <- pw(dd$pA1 + dd$pA2,a,b) - pw(dd$pA2,a,b)
	wA0 <- pw(dd$pA0 + dd$pA1 + dd$pA2,a,b) - pw(dd$pA1 + dd$pA2,a,b)

	wB2 <- pw(dd$pB2,a,b)
	wB1 <- pw(dd$pB1 + dd$pB2,a,b) - pw(dd$pB2,a,b)
	wB0 <- pw(dd$pB0 + dd$pB1 + dd$pB2,a,b) - pw(dd$pB1 + dd$pB2,a,b)

	UA <- wA0 * rr(dd$A0,r) + wA1 * rr(dd$A1,r) + wA2 * rr(dd$A2,r)
	UB <- wB0 * rr(dd$B0,r) + wB1 * rr(dd$B1,r) + wB2 * rr(dd$B2,r)
