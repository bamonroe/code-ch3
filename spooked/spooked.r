# Analyze the spooked data

load("../data/spooked/spookedswitching10.Rda")

D <- D[,c("id","period","hochoice","pp0left","pp1left","pp0right","pp1right","lsafe","lrisky","rsafe","rrisky")]
colnames(D) <- c("id","lotid","c","pA0","pA1","pB0","pB1","A0","A1","B0","B1")
D <- D[order(D$id),]

D$c <- ifelse(D$c==2,1,0)
