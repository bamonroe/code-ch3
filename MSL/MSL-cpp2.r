...
do.optim0 <- function(h,int){
	...
	m <- optim(par=int,fn=MSL0,h=h,h1=h1,h2=h2,method="BFGS", control=con,hessian=TRUE )
...
do.optim2 <- function(h,int){
	...
	m <- optim(par=int,fn=MSL2,h=h,h1=h1,h2=h2,method="BFGS", control=con,hessian=TRUE )
...
m0 <- do.optim0(int=init,h=h )
m2 <- do.optim2(int=init,h=h )
